...
namespace Chess.Game {
	public class HumanPlayer : Player {
		...
		public HumanPlayer (Board board) {
			...
			if (boardUI != null)
			{
				Debug.Log("I Found UI");
			}
			else
			{
                Debug.Log("Ghanta Mila Mujhe");
            }
			cam = Camera.main;
			this.board = board;
		}
		...
		public override void Update () {
			if (GameManager.Instance.upgradePawnPanel.active == false)
			{
                HandleInput();
            }
		}
		...
		void TryMakeMove(Coord startSquare, Coord targetSquare)
		{
			...
			if (moveIsLegal)
			{
				...
				{
					GameManager.Instance.OpenUpgradePawnPanel(chosenMove , () => { currentState = InputState.None; });
		...
		public void MoveItForMe(Move move)
		{
			ChoseMove(move);
		}
